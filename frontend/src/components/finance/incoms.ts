...
export class Incoms {
    ...
    async choiceDelCategory(e:any):Promise<void> {
        if (this.commonParams) {
            const result:DefaultResponseType = await HttpUtils.request('/categories/income/' + this.commonParams.currents.currentIncomeCtg, 'DELETE');
            if (!result.error) { await this.openNewRoute('/incoms'); }
        }
    }


    private toDeleteCategory(element:any):void {
    ...
    private toEditCategory(element:any):void {
        if (!this.commonParams) return;
        const choiceCtg = element.srcElement.getAttribute('choice-ctg-id');
        const choiceCtgName = element.srcElement.getAttribute('choice-ctg-name');
